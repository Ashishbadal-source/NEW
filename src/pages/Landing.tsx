...
export default function Landing() {
  ...
  // ✅ Updated handleDemoSubmit with API call
  const handleDemoSubmit = async (e: React.FormEvent) => {
    e.preventDefault();

    try {
      const res = await fetch("http://localhost:5001/api/demoRequest", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const features = [
    {
      ...
      title: "Real-time Risk Maps",
      description:
        "Interactive visualization of rockfall risk zones with live updates from sensor networks.",
      color: "text-primary",
    },
    {
      icon: <Activity className="w-6 h-6" />,
      title: "AI Predictions",
      description:
        "Advanced machine learning models analyze multiple data sources for accurate forecasting.",
      color: "text-risk-low",
    },
    {
      icon: <AlertTriangle className="w-6 h-6" />,
      title: "Instant Alerts",
      description:
        "Automated notifications via SMS and email when risk thresholds are exceeded.",
      color: "text-risk-high",
    },
    {
      icon: <Zap className="w-6 h-6" />,
      title: "24/7 Monitoring",
      description:
        "Continuous monitoring with real-time data processing and analysis.",
      color: "text-warning",
    },
    {
      icon: <BarChart3 className="w-6 h-6" />,
      title: "Comprehensive Analytics",
      description:
        "Detailed reports and insights to optimize safety protocols and operations.",
      color: "text-info",
    },
    {
      icon: <Users className="w-6 h-6" />,
      title: "Role-based Access",
      description:
        "Tailored interfaces for operators, inspectors, and administrators.",
      color: "text-accent",
    },
  ];
      ...
      <section id="features" className="py-20 bg-gradient-dark">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
              Leveraging cutting-edge AI and real-time data to prevent rockfall
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {features.map((feature, index) => (
              <Card
                key={index}
                className="bg-card hover:bg-card-hover transition-all duration-300 border-border"
              >
                <div className="p-6">
                  <div className={`${feature.color} mb-4`}>{feature.icon}</div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <Input
                      value={demoForm.mineRegion}
                      onChange={(e) =>
                        setDemoForm({ ...demoForm, mineRegion: e.target.value })
                      }
                      className="bg-input border-border"
                      placeholder="e.g., Western Australia"
                    />
